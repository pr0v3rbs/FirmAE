...
BASIC_CRED = {
    "asus": (b"admin", b"admin"),
    "belkin": (b"admin", b""),
    "dlink": (b"admin", b""),
    "netgear": (b"admin", b"password"),
    "linksys": (b"admin", b"admin"),
    "tplink": (b"admin", b"admin"),
    "trendnet": (b"admin", b""),
    "zyxel": (b"admin", b"1234"),
}
...
def HNAP_AUTH(SOAPAction, privateKey):
    ...
    b = str(b)[:-2]
    h = (
        hmac_md5(privateKey, b + '"http://purenetworks.com/HNAP1/' + SOAPAction + '"')
        .hexdigest()
        .upper()
    )
    return h + " " + b
...
def check_login_type(ip):
    ...
    if r.status_code == 401:
        ...
        r.text.find("log_pass") != -1 or r.text.find("login_auth.asp") != -1
    ):  # dlink normal login
        return "dlink_asp"
    elif r.text.find("setup_top.htm") != -1:
        return "belkin"
    elif (
        r.text.find("location.replace('login.htm')") != -1
        or r.text.find("login.ccp") != -1
    ):
        return "trendnet_ccp"
    else:
        return "unknown"
...
def get_session(brand, ip):
    ...
    if login_type == "basic":
        ...
        payload = (
            '<?xml version="1.0" encoding="utf-8"?><soap:Envelope xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/"><soap:Body><Login xmlns="http://purenetworks.com/HNAP1/"><Action>login</Action><Username>Admin</Username><LoginPassword>'
            + md5_password
            + "</LoginPassword><Captcha></Captcha></Login></soap:Body></soap:Envelope>"
        )
        requests.post(
            "http://" + ip + "/HNAP1/", headers=headers, data=payload, cookies=cookies
        )
        ...
        headers["Cache-Control"] = "max-age=0"
        payload = {
            "totalMSec": "1539994224.535",
            "pws": "d41d8cd98f00b204e9800998ecf8427e",
            "arc_action": "login",
            "pws_temp": "",
            "action": "Submit",
        }
